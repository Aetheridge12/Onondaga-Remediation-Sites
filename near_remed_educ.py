...
# Final Project - Block Group Education Data - Proximity to Remediation Site
# 5/3/2021
...
bg_educ_data = pd.read_csv("onondaga_bg_educ.csv")

#%%

# Setting the GEOID to string and making it the index

bg_educ_data = bg_educ_data.astype({"GEOID":str})

bg_educ_data = bg_educ_data.set_index("GEOID")
...
# Creating empty df - by_educ

by_educ = pd.DataFrame()

#%%

# Adding a column to by_educ df that holds the estimated number of 
# residents with hs diploma, ged, or lower in each radius of a block group

by_educ['no_college'] = stacked * bg_educ_data['no_college']

#%%

# Repeating the above step to estimate the amount of people who have more than
# a hs degree or ged in each radisu of a block group

by_educ['college'] = stacked * bg_educ_data['college']
...
by_educ_ring = by_educ.sum(level=1)
...
fig.suptitle("Probability of Radius Level, by Educational Attainment")

prob.plot.bar(y=['no_college', 'college'], ax=ax1)
...
fig.tight_layout()
fig.savefig("prob_by_educ_remed.png")
...
fig.suptitle("Odds Ratio of Location, No College to College")
...
fig.tight_layout()
fig.savefig("educ_ratio_remed.png")
